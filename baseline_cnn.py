...
IMPLEMENTED here:
  * audio -> mel-spectrogram -> CNN -> real/fake pipeline
  * EER metric on a validation split carved from train
  * best-checkpoint saving by validation EER
  * dB-safe padding (pads with the spectrogram minimum, i.e. silence)

PLANNED (not yet implemented — described as future work in the proposal):
  * speaker/source-disjoint splitting (needs group metadata beyond the CSV)
  * class weighting / balanced sampling for the ~70-82% spoof imbalance
  * evaluation on the official test split (ds["test"]) + F1 / confusion matrix
  * explicit spectrogram params, seeds, and robust file-loading
...
class MelSpecDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # IMPORTANT: in dB, 0 == loudest, so padding with zeros would insert
        # artificial high-energy regions. Pad with the clip's minimum (silence).
